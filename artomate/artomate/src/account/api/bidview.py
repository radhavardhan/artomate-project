...
class BidRequest(APIView):
    ...
    def post(self, request):
        if request.method == 'POST':
            ...
            project_bid = PostProject.objects.filter(project_code=project_code).values('id', 'project_title',
                                                                                       'description', 'min', 'max',
                                                                                       'username', 'created_at',
                                                                                       'route', 'userid')
                                ...
                                if bid.exists():
                                    ...
                                    if id in mylist:
                                        ...
                                        if serializer.is_valid():
                                            ...
                                            for e in user_bid:
                                                j = e['bid'] - 1
                                                biduseremail=e['email']
                                                user_bid.update(bid=j)
                                            bids.save()
                                            # subject, from_email, to ="one bid for ur project", 'testartomatestudio @ gmail.com', [i['email']]
                                            # text_content = 'This is an important message.'
                                            # html_content = render_to_string('ticket_active_email.html')
                                    ...
                                    if serializer.is_valid():
                                        ...
                                        for e in user_bid:
                                            ...
                                            user_bid.update(bid=j)
                                        bids.save()
                                        # subject, from_email, to = "one bid for ur project", 'testartomatestudio @ gmail.com', [
                                        #     i['email']]
                                        # text_content = 'This is an important message.'
                                        # html_content = render_to_string('ticket_active_email.html')
                                        # msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
...
class HirerProjects(APIView):
    ...
    def get(self, request):
        ...
        projects = PostProject.objects.filter(userid=id).values('id', 'project_title', 'description', 'min', 'max',
                                                                'username', 'created_at', 'route', 'project_deadline',
                                                                'custom_budget').order_by('created_at').reverse()
        for i in projects:
            bidstatus = Bidproject.objects.filter(project_id=i['id']).filter(bid_status="Accepted").values('email',
                                                                                                           'bid_amount',
                                                                                                           'descreption')
...
class Bid_Details_Project(APIView):
    ...
    def get(self, request, projectroute):
        ...
        data1 = {}
        mylist = []
        projects_posted = PostProject.objects.filter(route=projectroute).filter(userid=id).values('id', 'route',
                                                                                                  'project_deadline',
                                                                                                  'min', 'max')
        if projects_posted.exists():
            ...
            for i in projects_posted:
                ...
                biddeatils = Bidproject.objects.filter(project_id=i['id']).values('bid_amount', 'user_id',
                                                                                  'completion_time', 'email')
                # print(biddeatils)
                for j in biddeatils:
                    userdetails = Userprofile.objects.filter(user_id=j['user_id']).values('user_name', 'profile',
                                                                                          'country_id', 'hourely_rate')
                    for k in userdetails:
                        data = {"username": k['user_name'], "profile": k['profile'],
                                "location": country.objects.filter(id=k['country_id']).values('country_name'),
                                "email": j['email'], "bidamount": j['bid_amount'], "review": 3,
                                "hourely": k['hourely_rate'], "completiontime": j['completion_time'],
                                "userid": j['user_id']}
...
class No_Of_Bid(APIView):
    ...
    def get(self, request):
        ...
        for var in biddetails:
            projectdetails = PostProject.objects.filter(id=var['project_id']).values('id', 'project_title',
                                                                                     'description', 'min', 'max',
            ...
            if projectroute123.exists():
                for j in projectroute123:
                    for i in account:
                        ...
                        if bid.exists():
                            biduser = Bidproject.objects.filter(project_id=j['id']).filter(
                                bid_status="Accepted").values()
                            if biduser.exists():
                                ...
                                for p in projectroute123:
                                    details = []
                                    for i in account:
                                        bida_ccept_details = {
                                            "freelancer_id": i['id'],
                                            "hierer_id":employerid,
                                            "freelancer_message":"",
                                            "hierer_message": "hi",
                                            "isWho":1,
                                    ...
                                    details.append(bida_ccept_details)
                                    print(details.append(bida_ccept_details))
                                textdata = json.dumps(bida_ccept_details)
